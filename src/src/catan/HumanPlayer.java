 ...
 *
 * Why inheritance over a PlayerAgent interface:
 *   HumanPlayer Is A Player — it shares all game state (resources,
 *   roads, buildings). Only action-selection behaviour differs, so
 *   extending Player avoids duplicating state while preserving full
 *   substitutability in Game and Rules (Liskov Substitution Principle).
 *
 * Turn flow:
 *   Human turn : Roll → (List | Build)* → Go
 *   AI turn    : game executes normally, then waitForGo() pauses until
 *                the human types "go" to step forward (R2.4).
 *
 * Output format: [TurnID] / [PlayerID]: [Action]  via Login.printTurnAction
 *
 * @author Rameen Tariq
 */
public class HumanPlayer extends Player {
    ...
    /**
     * Constructs a HumanPlayer with injected I/O dependencies.
     *
...
/**
 * Constrcts a HumanPLayer with custom CommandParser for testing
 * @param playerId     unique player identifier
 * @param colour       colour assigned to this player
 * @param inputHandler abstraction over the input/output source
...
public HumanPlayer(int playerId, Colour colour, InputHandler inputHandler, CommandParser parser) {
    super(playerId, colour);
    if (inputHandler == null) throw new IllegalArgumentException("InputHandler must not be null.");
    if (parser == null)       throw new IllegalArgumentException("CommandParser must not be null.");
    this.inputHandler = inputHandler;
    this.parser       = parser;
}
    ...
    while (!turnOver) {
        ...
        switch (cmd.type) {
            ...
            case ROLL:
                ...
                Login.printTurnAction(roundNumber, getPlayerId(), "Rolled " + roll);
                if (roll == 7) {
            ...
            case BUILD_SETTLEMENT:
                if (!hasRolled) { inputHandler.displayMessage("You must Roll before building."); break; }
            ...
            case GO:
                if (!hasRolled) { inputHandler.displayMessage("You must Roll before ending your turn."); break; }
                Login.printTurnAction(roundNumber, getPlayerId(), "Passed");
                turnOver = true;
                break;
...
//Step-forward (R2.4)
/**
 * Waits for the human to type "go" before the game proceeds to the next turn.
 * Called by Game after each AI player's turn completes.
 *
...
private void handleBuildSettlement(int roundNumber, Board board, Rules rules, int nodeId) {
    ...
    Login.printTurnAction(roundNumber, getPlayerId(), "Built settlement at intersection " + nodeId);
}
...
private void handleBuildCity(int roundNumber, Board board, Rules rules, int nodeId) {
    ...
    addVictoryPoints(1);
    Login.printTurnAction(roundNumber, getPlayerId(), "Upgraded to city at intersection " + nodeId);
}
...
private void handleBuildRoad(int roundNumber, Board board, Rules rules, int fromNode, int toNode) {
    ...
    Login.printTurnAction(roundNumber, getPlayerId(),
...
private boolean hasNoRoadsYet(Board board) {
    for (Edge e : board.getAllEdges()) {
        if (e.getRoadOwnerId() == getPlayerId()) {
            return false;
        }
    }
    return true;
